...
namespace DataBaseService.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public ActionResult<OrderReadDto> Get(int id)
        {
            if (id <= 0)
            {
            ...
            return Ok(_mapper.Map<OrderReadDto>(t));
        }
        ...
        [ProducesResponseType(200)]
        [ProducesResponseType(400)]
        public ActionResult Post(OrderCreateDto dto)
        {
            try
            {
                _repo.Create(_mapper.Map<Order>(dto));
                _repo.SaveChanges();

                _logger.Write(NLog.LogLevel.Trace, $"{ToString()}.Create DeliveryAddressId:{dto.DeliveryAddressId}  Ok");
                return Ok();
            }
            catch (Exception ex)
            {
                _logger.Write(NLog.LogLevel.Error, $"{ToString()}.Create DeliveryAddressId:{dto.DeliveryAddressId}. {ex.Message}");
                return BadRequest();
            }
        }
        ...
        [ProducesResponseType(400)]
        [ProducesResponseType(404)]
        public ActionResult Put(int id, OrderCreateDto dto)
        {
            ...
            try
            {
                _repo.Update(id, _mapper.Map<Order>(dto));
                _repo.SaveChanges();
